...
sim_power_anova <- function(x){
  A <- rnorm(n=26,mean=0, sd=1)
  B <- rnorm(n=26,mean=0, sd=1)
  C <- rnorm(n=26,mean=0, sd=1)
  D <- rnorm(n=26,mean=(0+x), sd=1)
  df <- data.frame(condition = as.factor(rep(c("A","B","C","D"),each=26)),
                   DV = c(A,B,C,D))
